...
- Route-table connector lines must leave the route-table card from the colored header band, not from the white body. When several lines leave the same route-table or DRG route-table card, use one shared origin point for that card so the fan-out reads as a single association group.
...
- Add a clickable documentation icon in the legend for the hub model represented by the source file. Keep the icon plus label inside the legend box, aligned toward the right side of the lower legend band, with enough padding so it does not touch the glossary text or the legend border.
