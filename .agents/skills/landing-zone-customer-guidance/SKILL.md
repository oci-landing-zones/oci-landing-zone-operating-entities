...
- After establishing the One-OE baseline, ask: **Do you want to deploy a Disaster Recovery (DR) region?** If the customer answers yes, ask for the home region and the DR region explicitly. The current published Factory preset is the default example for `eu-frankfurt-1` as the home region and `eu-amsterdam-1` as the DR region, and it deploys only the DR hub and `prod` VCN using `10.0.192.0/21` and `10.0.200.0/21`, respectively. However, the design should accept other supported region pairs as long as the resulting topology is reviewed; do not assume a fixed region pair or apply the default preset to Multi-OE without review.
