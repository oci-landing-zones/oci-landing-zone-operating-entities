...
- `config_params.create_fss` is a boolean and defaults to `false`. When enabled, the extension adds a private FSS subnet, service-gateway-only route table, ICMP security list, FSS NSG, paired stateless NFS rules between the FSS and worker NSGs, paired stateless NFS rules between the FSS and pod NSGs for native CNI and virtual-node access, and the cluster-principal `manage file-family` statement.
