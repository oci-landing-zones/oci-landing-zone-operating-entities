...
- `gen/workload-extensions/ocvs/single-stack/output_builder.libsonnet` — owns the profile-to-committed-JSON output surface for published OCVS artifacts while the generic `ocvs_simple` extension remains config-mode owned.
...
- `gen/workload-extensions/ocvs/ocvs_simple.libsonnet` is the active generic extension wrapper for config mode and integrated landing-zone assembly.
