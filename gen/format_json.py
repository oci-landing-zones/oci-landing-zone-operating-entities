...
def calculate_max_key_length(obj: Dict[str, Any]) -> int:
    """
    Calculate the maximum key length in an object for colon alignment.
    Includes quotes around the key.
    """
    if not obj:
        return 0
    return max(len(json.dumps(key)) for key in obj.keys())
...
def format_array(arr: List[Any], indent_level: int) -> str:
    """
    Format an array with proper indentation.
    Objects inside arrays also get aligned colons.
    """
    if not arr:
        return "[]"
    ...
    for i, item in enumerate(arr):
        if isinstance(item, dict):
            ...
            formatted_obj = format_object(item, indent_level + 1)
            lines.append(next_indent + formatted_obj + ("," if i < len(arr) - 1 else ""))
        elif isinstance(item, list):
            # Nested array
            formatted_arr = format_array(item, indent_level + 1)
...
def format_object(obj: Dict[str, Any], indent_level: int) -> str:
    """
    Format a JSON object with aligned colons.
    Each object independently calculates its max key length for alignment.
    """
    if not obj:
        return "{}"
    ...
    # Calculate max key length for this object
    max_key_len = calculate_max_key_length(obj)
    ...
    for i, key in enumerate(sorted_keys):
        ...
        spacing = " " * (max_key_len - len(key_json))
        ...
        if isinstance(value, dict):
            ...
            formatted_value = format_array(value, indent_level + 1)
        else:
            formatted_value = format_primitive(value)

        # Build the line with aligned colon
...
def format_json(data: Any) -> str:
    ...
    if isinstance(data, dict):
        result = format_object(data, indent_level=0)
    elif isinstance(data, list):
        result = format_array(data, indent_level=0)
    else:
