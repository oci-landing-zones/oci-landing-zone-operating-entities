...
local common = (import '../../../../addons/oci-hub-models/hub_common.libsonnet');
...
function(mgmt_cidr, lb_cidr, has_spoke_natgw=false) {
  ...
  local spokes = [
    { name: 'prod', display: 'Prod', vcn: ip.prod_vcn, priority: 10 },
    { name: 'preprod', display: 'PreProd', vcn: ip.preprod_vcn, priority: 20 },
  ],
  ...
  _drg_spoke_attachments:: {
    ['DRGATT-FRA-LZ-%s-PROJ-KEY' % std.asciiUpper(s.name)]: {
      ...
      network_details: {
        attached_resource_key: 'VCN-FRA-LZ-%s-PROJECTS-KEY' % std.asciiUpper(s.name),
        type: 'VCN',
      },
    }
    for s in spokes
  ...
  _preprod_category:: spoke_category('preprod',
    if has_spoke_natgw then peers_for(spokes[1]) else []),
}
