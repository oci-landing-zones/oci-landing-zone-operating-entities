# Generic Multi-OE with Hub B

## Overview

[Hub B](/addons/oci-hub-models/hub_b/readme.md) routes north-south and east-west traffic through one OCI Network Firewall. It is a production-capable option with a consolidated inspection tier.
...
![Hub B design](/addons/oci-hub-models/hub_b/images/hub_b_design.png)
...
|---|---|---|
| 1 | `multioe_iam.json`<br>`multioe_governance.json`<br>`multioe_network_hub_b_pre.json`<br>`multioe_security_cis1_pre.json`<br>`multioe_observability_cis1_pre.json` | Replace network, security, and observability pre files with `multioe_network_hub_b.json`, `multioe_security_cis1.json`, and `multioe_observability_cis1.json` |
| 2 | `multioe_iam.json`<br>`multioe_governance.json`<br>`multioe_network_hub_b_pre.json`<br>`multioe_security_cis2_pre.json`<br>`multioe_observability_cis2_pre.json` | Replace network, security, and observability pre files with `multioe_network_hub_b.json`, `multioe_security_cis2.json`, and `multioe_observability_cis2.json` |
...
4. Obtain the deployed firewall Private IP OCID.
5. Replace every full example `network_entity_id` value in `multioe_network_hub_b.json` with the firewall Private IP OCID.
