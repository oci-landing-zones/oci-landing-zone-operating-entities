...
## Generated One-Stack Runtime

The modern generated runtime lives in [`one-stack/`](./one-stack/). Its JSON files are generated from Jsonnet sources under `gen/blueprints/multi-oe/generic_v1/runtime/one-stack/`.

The existing `op01` through `op04` folders are legacy operation-split assets and are not regenerated by the blueprint factory.
