...
#### Step 1: Create ORM Stack <!-- omit from toc -->
...
2. **Configure Stack Source**
   ...
   - Choose **Source**: `.zip file`
   - Upload a ZIP containing:
     - [Orchestrator module](https://github.com/oci-landing-zones/terraform-oci-modules-orchestrator) Terraform files
     - All four `*.auto.tfvars.json` files from `single-stack/` directory

3. **Set Stack Variables**
   - `tenancy_ocid`: Auto-populated from your session
   - `region`: Your target region (e.g., `eu-frankfurt-1`)
   - The orchestrator will automatically load all `.auto.tfvars.json` files

4. **Review Configuration** (Optional Customization)
...
#### Step 2: Plan and Apply <!-- omit from toc -->
...
2. **Review Plan Output**
   - Verify ~200+ resources will be created
   - Check for any errors or warnings

3. **Apply Configuration**
   - Click **Apply**
   - Deployment takes approximately **30-40 minutes**
  ...
  "clusters_configuration": {
    "clusters": {
      "OKE-FRA-P-PLATFORM-KEY": {
        "kubernetes_version": "v1.30.1",  // Updated version
        ...
      }
    }
  }
