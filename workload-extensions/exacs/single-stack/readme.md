...
The listed foundation files establish the Landing Zone prerequisites. The published UC1 database workload is optional and is intentionally absent from the public convenience buttons because it contains secret and SSH-key placeholders. After replacing those values in a customer-controlled package, add [exacs_cloud_exadata_database.json](../database-workload/single-stack/exacs_cloud_exadata_database.json) to the UC1 foundation operation; it contains one `cloud_exadata_database_configuration` root with every Exadata section and therefore does not collide with the foundation families. On the final observability re-apply, retain that workload JSON unchanged together with every unchanged foundation document, replacing only the pre observability document with its final counterpart. For the full procedure, see the [Cloud Exadata Database workload deployment guide](../database-workload/readme.md).
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Landing Zone, network, platform, security, shared ExaDB-D, environment, project, and project database compartments.<br><br>Base One-OE IAM plus Global Infra, Global DBA, and Project DBA groups and policies, including read access from each Project DBA to its hosting ACD.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>• Landing Zone and ExaDB-D compartments<br>• Landing Zone and ExaDB-D IAM groups and policies</td>
      <td>Landing Zone, network, platform, security, environment-dedicated ExaDB-D, project, and project database compartments.<br><br>Base One-OE IAM plus environment Infra and DBA groups per environment and Project DBA groups and policies. No global ExaDB-D administration group is generated.</td>
    </tr>
    <tr>
