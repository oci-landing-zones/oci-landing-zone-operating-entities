locals {
  operation_stage = upper(var.external_database_management_configuration.operation_stage)
  targets = {
    for key, target in var.external_database_management_configuration.targets : key => merge(target, {
  ...
  cdb_targets     = { for key, target in local.targets : key => target if target.database_type == "CDB" }
  pdb_targets     = { for key, target in local.targets : key => target if target.database_type == "PDB" }
  non_cdb_targets = { for key, target in local.targets : key => target if target.database_type == "NON_CDB" }
}
