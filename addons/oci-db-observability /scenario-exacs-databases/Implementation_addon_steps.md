...
1. From the EXACS scenario README, click the ORM deployment button for the GLOBAL approach. The button opens a new ORM stack with the JSON files for that option preloaded.
...
6. ORM will load the required JSON files. This ExaCS scenario does not deploy a separate VM for the Logging Analytics agent.
