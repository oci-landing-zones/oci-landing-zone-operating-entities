...
1. From the ExaDB-C@C scenario README, click the ORM deployment button for the CENTRALIZED approach. The button opens a new ORM stack with the JSON files for that option preloaded.
...
6. ORM will load the required JSON files. This ExaDB-C@C scenario does not deploy a separate VM for the Logging Analytics agent.
