...
4. Create the private endpoint for Database Management. Use the Global subnet and NSG model from the Observability Landing Zone add-on deployment. The ODA@ database hosts are in `vcn-fra-lz-shared-oda`, subnet `sn-fra-lz-shared-oda-db`.
...
6. Verify connectivity between the target database and the private endpoint.
   ...
   The add-on creates the required NSGs for the Global model. Confirm the target database uses `sn-fra-lz-shared-oda-db`, confirm the service private endpoints use the expected subnet and NSG assignments, and verify that the private endpoint network can reach the target ODA@ SCAN listener on port `1521`.

## Enable Database Management for ODA@

For each database you want to enable:

Open the target ODA@ database details page in the OCI Console.

The database details page shows the associated service links for Database Management and Operations Insights.

No scenario-specific database details screenshot is currently included for ODA@.
...
For each Pluggable Database, open the target ODA@ database details page and then open the Pluggable Database details.

To identify the PDB service name, use `lsnrctl status`.

## Enable Operations Insights for ODA@

Operations Insights can be enabled for ODA@, CDB, and PDB resources in a single flow.
...
No scenario-specific Ops Insights add database screenshot is currently included for ODA@.

## Enable Logging Analytics for ODA@

ODA@ logs contain information that should be included in a complete observability design. To analyze these logs in OCI Logging Analytics, push them into Logging Analytics. This guide uses Management Agents installed on the ODA@ database hosts.
