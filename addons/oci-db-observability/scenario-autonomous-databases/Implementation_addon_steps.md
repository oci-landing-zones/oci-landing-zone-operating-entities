...
For CENTRALIZED, to grant the `grp-lz-global-mon-admin` group in the common One-OE identity domain access to specific resources, you can optionally replace the vault and key statements in the `pcy-centralized-mon-security-admin` policy with statements scoped to specific Vault and Key OCIDs. Check the IAM file selected for your deployment option.

```
allow group 'id_lz_common'/'grp-lz-global-mon-admin' to use vaults in compartment cmp-landingzone:cmp-lz-security where target.vault.id='ocid1.vault.oc1.region.xxxx'
allow group 'id_lz_common'/'grp-lz-global-mon-admin' to use keys in compartment cmp-landingzone:cmp-lz-security where target.key.id='ocid1.key.oc1.region.xxx'
```
