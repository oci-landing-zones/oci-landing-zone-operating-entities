...
6. ORM will load the required JSON files. SSH access is optional. If you need SSH access to the monitoring instance, provide your own version of the selected instance JSON file (`addon_obs_instance_atp_centralized.json` or `addon_obs_instance_project.json`) and set `default_ssh_public_key_path` to the full SSH public key value. A file path only works if that file is available to the Terraform run.
