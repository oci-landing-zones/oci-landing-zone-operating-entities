...
- In **multi-region** landing zones, **identity and security** operate on two levels: Global IAM resources such as compartments, groups, users, and policies are managed from the home region and should be reused consistently across subscribed regions, while regional resources—including security resources (vaults, keys, secrets) and workload-specific security controls—must be explicitly provisioned and deployed or replicated in each target region as part of the workload recovery plan.
...
```mermaid
flowchart LR
    HC["Home Blueprint Factory config<br/>complete scope + RPC acceptor"] --> HG["generate.sh --config"]
    DC["DR Blueprint Factory config<br/>regional scope + RPC requester"] --> DG["generate.sh --config"]
    HG --> HS["Home ORM stack / Terraform state"]
    DG --> DS["DR ORM stack / Terraform state"]
    HS -->|network dependency| DS
```

<p align="center"><em>Figure 2</em></p>

<p align="left"><strong>Figure 2: Independently generated multi-stack deployment</strong></p>
...
3. Select and follow the appropriate deployment guide below. You MUST use the same hub model as the home-region deployment; mixed hub models are not supported by this add-on. You MUST select the CIS level required by the DR design. For a custom same-hub scenario not listed below, use the [OCI LZ Blueprint Factory](../oci-lz-blueprint-factory/README.md) to create your custom JSON configuration files.
...
> [!NOTE]
> This add-on covers the DR Landing Zone scope, including core infrastructure resources such as VCNs, route tables, security resources, events, notifications, and other core Landing Zone components. It does not deploy workload-specific resources, such as databases, OKE clusters, Exadata Cloud Service (ExaCS), Compute instances, or other application-specific resources.
