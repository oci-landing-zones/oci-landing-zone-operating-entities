...
It complements the OCI LZ BCDR addon documentation. The addon deploys the regional landing zone resources for DR; workload replication, failover procedures, and service-specific recovery configuration must be implemented separately according to the selected DR strategy.
...
| **Remote Peering Connections, DRG attachments, and inter-region routes** | Regional inter-region connectivity | BCDR addon | Keep each region's RPC resources in its canonical network configuration. Apply the home network first, publish its dependency output, and then apply the DR network. Validate connectivity in both directions. Cross-tenancy peering requires the appropriate tenancy permissions. |
...
For CIS Level 2 Vault replication, the home-owned IAM policy must authorize the Vault service to manage the replicated vault. Prefer a reviewed compartment-scoped statement for the security compartment rather than a tenancy-wide grant, and verify that the selected vault reports `isVaultReplicable` before creating the replica.
...
The BCDR addon provides the regional landing zone resources but does not define or guarantee RTO or RPO for individual workloads. Workload owners must set these objectives, select a recovery strategy that meets them, measure the results during recovery tests, and approve any remaining gaps.
...
The landing zone extension prepares the regional DR resources. Configure and validate workload replication separately, based on the OCI service used and the selected recovery objectives.
...
- **BCDR addon**: deploy and maintain the regional network, VSS, observability, and inter-region connectivity resources in the DR region.
...
After deploying the regional landing zone resources and configuring workload recovery, validate the following items and record the results in the DR runbook:
