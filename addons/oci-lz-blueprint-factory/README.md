...
Deploy `generated/oneoe-dr/home/` and `generated/oneoe-dr/dr/` with different OCI Resource Manager stacks or Terraform states. Both regional configurations must use the same hub model. Blueprint Factory supports Hub A, Hub B, Hub C, and Hub E pairs, but does not support combining different hub models in one DR pair.
